...
8. PRESENT    → Display formatted answers ready to copy-paste
```
...
4. If Section G exists → load the prior draft answers as a starting point
5. If no match found → warn and offer to run a quick auto-pipeline first
...
- **If re-evaluating**: Run full evaluation, update report, regenerate Section G
...
- Yes/No (relocation, visa sponsorship, etc.)
...
Based on: Report #NNN | Match: XX% (TN) | Archetype: [type]
...
- [Personalization suggestions the candidate should review before submitting]
```
...
3. Suggest next step: `/career-ops contacto` for LinkedIn outreach to the hiring manager
