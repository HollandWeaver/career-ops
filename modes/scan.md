...
- Reads the page in real time (not cached Google results)
...
For companies using Greenhouse, the JSON API (`boards-api.greenhouse.io/v1/boards/{slug}/jobs`) returns clean structured data. Use as a fast complement to Level 1 — quicker than Playwright but only works with Greenhouse.
...
4. **Level 1 — Playwright scan** (parallel in batches of 3-5):
   ...
   g. If `careers_url` fails (404, redirect), try `scan_query` as fallback and note URL needs update
...
8.5. **Verify liveness of Level 3 WebSearch results** — BEFORE adding to pipeline:
   ...
   **Don't interrupt the entire scan if one URL fails.** If `browser_navigate` errors (timeout, 403, etc.), mark as `skipped_expired` and continue.
...
Each company in `tracked_companies` must have `careers_url` — the direct URL to their job listings page. This avoids searching for it each time.
