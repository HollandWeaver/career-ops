...
Each worker is a `claude -p` subprocess with a clean 200K token context. The conductor only orchestrates.
...
4. **For each pending URL**:
   ...
   d. Execute via Bash:
      ...
      ```
   e. Update `batch-state.tsv` (completed/failed + match % + report_num)
...
```
id	url	status	started_at	completed_at	report_num	match_pct	bucket	error	retries
1	https://...	completed	2026-...	2026-...	002	94	T2	-	0
...
Each worker receives `batch-prompt.md` as the system prompt. Self-contained.
